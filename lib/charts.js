...
async function getRenderer(width, height, version, format) {
  ...
  if (!rendererCache[key]) {
    ...
    rendererCache[key] = chartjsnodecanvas;
  }
...
async function renderChartJs(
  ...
  const ChartJSNodeCanvas = await getRenderer(width, height, version, format);

  if (format === 'svg') {
    // SVG rendering doesn't work asychronously.
    return Buffer.from(
      uniqueSvg(ChartJSNodeCanvas.renderToBufferSync(chart, 'image/svg+xml').toString()),
    );
  }
  return ChartJSNodeCanvas.renderToBuffer(chart);
}
